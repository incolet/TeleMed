...
public class ProviderAccount(IAppDbContext appDbContext, IConfiguration configuration) : AAccount(appDbContext,configuration)
{
    public override CustomResponses.LoginResponse LoginAsync(LoginDto model)
    {
        ...
        if (!IsUserInRole(findUser, (int)UserRoles.Provider))
        {
            return (new CustomResponses.LoginResponse(false, "User doesn't exist"));
        }
